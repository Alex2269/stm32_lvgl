...
{
  write_pin(LCD_CS_GPIO_Port, LCD_CS_Pin, LOW);
}

static void ILI9341_Unselect()
{
  write_pin(LCD_CS_GPIO_Port, LCD_CS_Pin, HI);
}
...
void ILI9341_SPI_Init(void)
{
  ...
  write_pin(LCD_CS_GPIO_Port, LCD_CS_Pin, LOW); // CS OFF
}

/* Send data (char) to LCD */
void ILI9341_SPI_Send(unsigned char SPI_Data)
{
  SPI_Transmit(HSPI_INSTANCE, &SPI_Data, 1);
}
...
{
  write_pin(LCD_CS_GPIO_Port, LCD_CS_Pin, LOW);
  write_pin(LCD_DC_GPIO_Port, LCD_DC_Pin, LOW);
...
{
  write_pin(LCD_DC_GPIO_Port, LCD_DC_Pin, HI);
  write_pin(LCD_CS_GPIO_Port, LCD_CS_Pin, LOW);
...
{
  write_pin(LCD_RST_GPIO_Port, LCD_RST_Pin, LOW);
  delay_ms(200);
  write_pin(LCD_CS_GPIO_Port, LCD_CS_Pin, LOW);
  delay_ms(200);
  write_pin(LCD_RST_GPIO_Port, LCD_RST_Pin, HI);
}
...
void ILI9341_Draw_Colour(uint16_t Colour)
{
  ...
  write_pin(LCD_DC_GPIO_Port, LCD_DC_Pin, HI);
  write_pin(LCD_CS_GPIO_Port, LCD_CS_Pin, LOW);
  SPI_Transmit(HSPI_INSTANCE, TempBuffer, 2);
  write_pin(LCD_CS_GPIO_Port, LCD_CS_Pin, HI);
}
...
void ILI9341_Draw_Colour_Burst(uint16_t Colour, uint32_t Size)
{
  ...
  if(Sending_in_Block != 0) {
    for(uint32_t j = 0; j < (Sending_in_Block); j++) {
      SPI_Transmit(HSPI_INSTANCE, (unsigned char *)burst_buffer, Buffer_Size);
    }
  }
  // REMAINDER!
  SPI_Transmit(HSPI_INSTANCE, (unsigned char *)burst_buffer, Remainder_from_block);
  write_pin(LCD_CS_GPIO_Port, LCD_CS_Pin, HI);
}
...
void ILI9341_Draw_Pixel(uint16_t X, uint16_t Y, uint16_t Colour)
{
  ...
  SPI_Transmit(HSPI_INSTANCE, Temp_Buffer, 4);
  write_pin(LCD_CS_GPIO_Port, LCD_CS_Pin, HI);
  ...
  SPI_Transmit(HSPI_INSTANCE, Temp_Buffer1, 4);
  write_pin(LCD_CS_GPIO_Port, LCD_CS_Pin, HI);
  ...
  SPI_Transmit(HSPI_INSTANCE, Temp_Buffer2, 2);
  write_pin(LCD_CS_GPIO_Port, LCD_CS_Pin, HI);
}
...
{
  write_pin(LCD_DC_GPIO_Port, LCD_DC_Pin, LOW);
  SPI_Transmit(HSPI_INSTANCE, &cmd, sizeof(cmd));
}
...
void ILI9341_WriteData(uint8_t * buff, size_t buff_size)
{
  ...
  while(buff_size > 0) {
    ...
    SPI_Transmit(HSPI_INSTANCE, buff, chunk_size);
    buff += chunk_size;
...
void ILI9341_Draw_Image(uint16_t x, uint16_t y, uint16_t width, uint16_t height, const char * Image_Array, uint8_t Orientation)
{
  ...
  for(uint32_t i = 0; i < width * height * 2 / BURST_MAX_SIZE; i++) {
    ...
    SPI_Transmit(SPI1, (unsigned char *)Temp_small_buffer, BURST_MAX_SIZE);
...
void ILI9341_Draw_Image(uint16_t width, uint16_t height, const char * Image_Array, uint8_t Orientation)
{
  ...
  for(uint32_t i = 0; i < width * height * 2 / BURST_MAX_SIZE; i++) {
    ...
    SPI_Transmit(&hspi1, (unsigned char *)Temp_small_buffer, BURST_MAX_SIZE);
    counter += BURST_MAX_SIZE;
  }
  write_pin(LCD_CS_GPIO_Port, LCD_CS_Pin, HI);
}
