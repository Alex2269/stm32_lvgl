...
* GLOBAL FUNCTIONS
**********************/
void lv_user_keypad_encoder(void)
{
  g = lv_group_create();
...
static void selectors_create(lv_obj_t * parent)
{
  ...
  lv_obj_set_size(btnmatrix, 380, 300);
  // lv_obj_set_content_width(btnmatrix, 400);
  // lv_obj_set_content_height(btnmatrix, 200);
  lv_obj_add_style(btnmatrix, &style_btn, 0);
  lv_obj_set_style_radius(btnmatrix, 24, 0);
  lv_obj_set_style_pad_row(btnmatrix, 10, LV_PART_MAIN);
  lv_obj_set_style_pad_column(btnmatrix, 10, LV_PART_MAIN);
  lv_obj_align(btnmatrix, LV_ALIGN_CENTER, 0, 30);
  lv_obj_add_event_cb(btnmatrix, event_cb, LV_EVENT_ALL, NULL);
  lv_obj_center(btnmatrix);
  lv_obj_add_flag(btnmatrix, LV_OBJ_FLAG_SCROLL_ON_FOCUS);
}
