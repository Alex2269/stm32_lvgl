...
static lcdPropertiesTypeDef lcdProperties =
{
  ILI9341_PIXEL_WIDTH, ILI9341_PIXEL_HEIGHT, LCD_ORIENTATION_PORTRAIT,true, true
};
...
lcdBuildMemoryAccessControlConfig(bool rowAddressOrder,
...
{
  lcdPortraitConfig =
  lcdBuildMemoryAccessControlConfig(
...
void lcdFillTriangle(int16_t x0, int16_t y0, int16_t x1, int16_t y1, int16_t x2, int16_t y2, uint16_t color)
{
  ...
  int16_t
  dx01 = x1 - x0, dy01 = y1 - y0, dx02 = x2 - x0, dy02 = y2 - y0, dx12 = x2 - x1, dy12 = y2 - y1;
  int32_t
  sa = 0, sb = 0;
...
void lcdSetOrientation(lcdOrientationTypeDef value)
{
  ...
  // cdWriteCommand(ILI9341_MEMORYWRITE);
  lcdSetWindow(0, 0, lcdProperties.width - 1, lcdProperties.height - 1);
}
